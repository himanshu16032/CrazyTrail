...
import { ArrowUpRight, Brain, Layers, Send } from 'lucide-react';
...
const CLUSTER_LABEL = {
  'Competitor Steal': 'Free alternatives to paid trend tools',
  Differentiator: 'How CrazyTrail finds trends early',
  Platform: 'Reels & Shorts topic alerts',
  Craft: 'Hooks & hashtags that still work',
  Concept: 'Timing the early viral window',
  Niche: 'Niche alerts you can film this week',
};
...
export default function TopicsIndex() {
  ...
  useSeo({
    ...
    keywords: 'free trend alerts, Treendly alternative free, Glimpse alternative free, free viral Reels ideas, YouTube Shorts content ideas free, free trend tool for Instagram creators',
    ...
    jsonLd: [
      {
        ...
        '@type': 'CollectionPage',
        name: 'CrazyTrail free trend guides',
        url: `${SITE_URL}/topics`,
        description: 'Free short-form trend guides that lead to CrazyTrail email alerts for Instagram Reels and YouTube Shorts creators.',
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <span className="inline-flex items-center gap-2 bg-primary/10 text-primary font-semibold px-4 py-1.5 rounded-full text-sm mb-4">
              <Layers className="w-4 h-4" /> Free creator guides
            </span>
            <h1 className="font-heading font-900 text-4xl sm:text-5xl md:text-6xl text-dark leading-tight mb-4">
              Find what to film next —{' '}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-accent-pink to-accent-orange">
                then get it in your inbox.
              </span>
            </h1>
            <p className="text-dark-light text-base md:text-lg max-w-3xl mx-auto">
              Short guides for Reels and YouTube Shorts. CrazyTrail’s job is simple: email you early niche topics 3–5 days before they peak — free, no card.
            </p>
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              <GradientButton
                onClick={() => {
                  track('topics_index_cta_click');
                  navigate('/?ref=topics');
                }}
              >
                <Send className="w-4 h-4" /> Get free trend alerts
              </GradientButton>
              <Link to="/methodology" className="inline-flex items-center gap-2 text-primary font-semibold text-sm hover:underline px-3">
                <Brain className="w-4 h-4" /> How the AI works
              </Link>
            </div>
          </motion.div>
          ...
          {steal.length > 0 && (
            <section className="mb-12 bg-white border border-primary/15 rounded-3xl p-5 md:p-8">
              <h2 className="font-heading font-bold text-xl text-dark mb-2">Looking for a free alternative?</h2>
              <p className="text-dark-light text-sm mb-5 max-w-3xl">
                Comparing Treendly, Glimpse, or Exploding Topics? These pages show when CrazyTrail is the better fit — free early Reels/Shorts alerts instead of paid search dashboards.
              </p>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {steal.map((t) => (
                  ...
                  >
                    <span className="text-[11px] font-semibold uppercase tracking-wide text-accent-orange">Free alternative</span>
          ...
          {CLUSTER_ORDER.filter((c) => grouped[c]).map((cluster) => (
            <section key={cluster} className="mb-12">
              <h2 className="font-heading font-bold text-xl text-dark mb-4">{CLUSTER_LABEL[cluster] || cluster}</h2>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5 mt-4 text-primary font-semibold text-sm">
                        Get early topics <ArrowUpRight className="w-4 h-4" />
                      </div>
                    </Link>
                  </motion.div>
                ))}
